...
if len(X_train) > TRAIN_SAMPLE_SIZE:
    print(f"Subsampling training data: {len(X_train)} -> {TRAIN_SAMPLE_SIZE} (stratified)")
    X_train, _, y_train, _ = stratified_sample(
        X_train, y_train, train_size=TRAIN_SAMPLE_SIZE,
...
print(f"Note: model trained on {len(X_train)} stratified-sampled rows, evaluated on full test set ({len(X_test)} rows)")

# ── Save model ────────────────────────────────────────────────────────────────
os.makedirs(MODELS_DIR, exist_ok=True)
joblib.dump({'model': model, 'scaler': scaler}, os.path.join(MODELS_DIR, 'knn.joblib'))
print(f"Model saved to {MODELS_DIR}/knn.joblib")
